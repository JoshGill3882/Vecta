...
- [ ] Markdown rendering: descriptions render in the task view modal (the detail view), not on list cards, via [`react-markdown`](https://github.com/remarkjs/react-markdown) with [`remark-gfm`](https://github.com/remarkjs/remark-gfm) for tables/strikethrough, sanitised with `rehype-sanitize`; links get safe `rel`/`target` handling
